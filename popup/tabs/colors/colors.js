...
class ColorsTab {
    ...
    analyzeColors() {
        chrome.tabs.query({active: true, currentWindow: true}, (tabs) => {
            chrome.tabs.sendMessage(tabs[0].id, {action: "analyzeColors"}, (response) => {
    ...
    updateColorUI(colorData) {
        ...
        if (document.getElementById('total-colors')) {
            ...
            document.getElementById('bg-colors').textContent = 
                this.getUniqueColors(colorData.background).length;
                
            document.getElementById('text-colors').textContent = 
                this.getUniqueColors(colorData.text).length;
        }
    }
    
    getUniqueColors(colors) {
        return [...new Set(colors)].filter(color => color && color !== 'transparent');
    }
